...
@pytest.fixture()
def wallet_interactor() -> UsersInteractor:
    return UsersInteractor(user_repo=InMemoryWalletRepository())
...
def test_users_interactor_generate_new_api_key_multiple_calls(users_interactor: UsersInteractor):
    for i in range(1000):
        first_user = users_interactor.generate_new_api_key()
        second_user = users_interactor.generate_new_api_key()
        assert first_user != second_user
